...
const { handleSingleUpload, handleMultipleUpload } = require("../config/multer");
...
// ── Helpers ──────────────────────────────────────────────────────────────────
const logConversion = async (req, tool, inputFiles, outputFile, status, errorMessage, startMs) => {
  try {
    await ConversionHistory.create({
      user: req.user?._id, sessionId: req.headers["x-session-id"],
      tool, status,
      inputFiles: (inputFiles || []).map((f) => ({ originalName: f.originalname || f.originalName, size: f.size, mimeType: f.mimetype || f.mimeType })),
  ...
  } catch (e) { logger.warn("History log failed:", e.message); }
};
...
const withSingle = (fn, tool) => async (req, res, next) => {
  ...
  try {
    ...
    const out    = { fileName: result.fileName, downloadUrl: buildDownloadUrl(req, result.fileName), size: result.size || stat.size, ...result };
    ...
    await logConversion(req, tool, req.file ? [req.file] : [], null, "failed", err.message, startMs);
    next(err);
  }
};
...
const withMultiple = (fn, tool) => async (req, res, next) => {
  ...
  try {
    ...
    const result = await fn(req.files.map((f) => f.path), req.body, req.files);
    const out    = { fileName: result.fileName, downloadUrl: buildDownloadUrl(req, result.fileName), size: result.size, ...result };
    delete out.outputPath;
...
const imageToPdf = async (req, res, next) => {
  ...
  try {
    ...
    const result = await pdfService.imagesToPdf(imagePaths, { pageSize, orientation, margin: parseInt(margin) });
    const stat   = await fse.stat(result.outputPath);
    const out    = { fileName: result.fileName, downloadUrl: buildDownloadUrl(req, result.fileName), size: stat.size };
    await logConversion(req, "image-to-pdf", req.files, out, "completed", null, startMs);
...
const pdfToWord = async (req, res, next) => {
  ...
  try {
    ...
    const stat   = await fse.stat(result.outputPath);
    const out    = { fileName: result.fileName, downloadUrl: buildDownloadUrl(req, result.fileName), size: stat.size };
    await logConversion(req, "pdf-to-word", [req.file], out, "completed", null, startMs);
    cleanup(req.file.path);
    success(res, out, "PDF converted to Word successfully");
  } catch (err) { if (req.file) cleanup(req.file.path); next(err); }
};
...
const wordToPdf = async (req, res, next) => {
  ...
  try {
    ...
    const stat   = await fse.stat(result.outputPath);
    const out    = { fileName: result.fileName, downloadUrl: buildDownloadUrl(req, result.fileName), size: stat.size };
    await logConversion(req, "word-to-pdf", [req.file], out, "completed", null, startMs);
    cleanup(req.file.path);
...
const pdfMerge = async (req, res, next) => {
  ...
  try {
    await handleMultipleUpload(req, res);
    if (!req.files || req.files.length < 2) return error(res, "Please upload at least 2 PDF files to merge", 400);
    const pdfPaths = req.files.map((f) => f.path);
    const result   = await pdfService.mergePdfs(pdfPaths);
    const stat     = await fse.stat(result.outputPath);
    const out      = { fileName: result.fileName, downloadUrl: buildDownloadUrl(req, result.fileName), size: stat.size };
    await logConversion(req, "pdf-merge", req.files, out, "completed", null, startMs);
    pdfPaths.forEach(cleanup);
    success(res, out, `${req.files.length} PDFs merged successfully`);
  } catch (err) { if (req.files) req.files.forEach((f) => cleanup(f.path)); next(err); }
};
...
const pdfSplit = async (req, res, next) => {
  ...
  try {
    ...
    const out = { fileName: zipResult.fileName, downloadUrl: buildDownloadUrl(req, zipResult.fileName),
      size: zipResult.size, pageCount: pageFiles.length };
    cleanup(req.file.path);
...
const pdfCompress = async (req, res, next) => {
  ...
  try {
    ...
    cleanup(req.file.path);
    success(res, { ...result, downloadUrl: buildDownloadUrl(req, result.fileName) }, "PDF compressed");
  } catch (err) { if (req.file) cleanup(req.file.path); next(err); }
};
...
const imageResize = async (req, res, next) => {
  try {
    ...
    cleanup(req.file.path);
    success(res, { ...result, downloadUrl: buildDownloadUrl(req, result.fileName) }, "Image resized");
  } catch (err) { if (req.file) cleanup(req.file.path); next(err); }
};
...
const imageCompress = async (req, res, next) => {
  try {
    ...
    cleanup(req.file.path);
    success(res, { ...result, downloadUrl: buildDownloadUrl(req, result.fileName) }, "Image compressed");
  } catch (err) { if (req.file) cleanup(req.file.path); next(err); }
};
...
const imageConvert = async (req, res, next) => {
  try {
    ...
    cleanup(req.file.path);
    success(res, { ...result, downloadUrl: buildDownloadUrl(req, result.fileName) }, "Image converted");
  } catch (err) { if (req.file) cleanup(req.file.path); next(err); }
};
...
const textToPdf = async (req, res, next) => {
  try {
    ...
    success(res, { ...result, downloadUrl: buildDownloadUrl(req, result.fileName) }, "Text converted to PDF");
  } catch (err) { next(err); }
};
...
const createZip = async (req, res, next) => {
  try {
    await handleMultipleUpload(req, res);
    if (!req.files?.length) return error(res, "No files uploaded", 400);
    const files  = req.files.map((f) => ({ filePath: f.path, archiveName: f.originalname }));
    const result = await compressionService.createZip(files);
    req.files.forEach((f) => cleanup(f.path));
    success(res, { ...result, downloadUrl: buildDownloadUrl(req, result.fileName) }, "ZIP created");
  } catch (err) { if (req.files) req.files.forEach((f) => cleanup(f.path)); next(err); }
};
...
const watermarkPdf    = withSingle((p, b) => advancedPdf.watermarkPdf(p, b), "watermark-pdf");
const signPdf         = withSingle((p, b) => advancedPdf.signPdf(p, b), "sign-pdf");
const redactPdf       = withSingle((p, b) => advancedPdf.redactPdf(p, { regions: JSON.parse(b.regions || "[]") }), "redact-pdf");
const addPageNumbers  = withSingle((p, b) => advancedPdf.addPageNumbers(p, b), "page-numbers");
...
const comparePdfs = async (req, res, next) => {
  ...
  try {
    ...
    const result = await advancedPdf.comparePdfs(req.files[0].path, req.files[1].path);
    req.files.forEach((f) => cleanup(f.path));
    success(res, { ...result, downloadUrl: buildDownloadUrl(req, result.fileName) }, "PDF comparison complete");
  } catch (err) { if (req.files) req.files.forEach((f) => cleanup(f.path)); next(err); }
};
...
const performOcr = async (req, res, next) => {
  ...
  try {
    ...
    const result = await ocrService.performOCR(req.file.path, { lang, outputFormat });
    cleanup(req.file.path);
    await logConversion(req, "ocr", [req.file], result, "completed", null, startMs);
    success(res, { ...result, downloadUrl: buildDownloadUrl(req, result.fileName) }, "OCR complete");
  } catch (err) { if (req.file) cleanup(req.file.path); next(err); }
};
...
const pdfToMarkdown = withSingle((p) => extConverters.pdfToMarkdown(p), "pdf-to-markdown");
...
const pdfToEpub     = withSingle((p, b) => extConverters.pdfToEpub(p, b), "pdf-to-epub");
const pdfToPptx     = withSingle((p) => extConverters.pdfToPptx(p),     "pdf-to-pptx");
const pdfToExcel    = withSingle((p) => extConverters.pdfToExcel(p),    "pdf-to-excel");
...
const markdownToPdf = withSingle((p) => extConverters.markdownToPdf(p), "markdown-to-pdf");
...
module.exports = {
  imageToPdf, pdfToWord, wordToPdf, pdfMerge, pdfSplit,
  pdfCompress, imageResize, imageCompress, imageConvert, textToPdf, createZip,
  // Advanced PDF
  pdfToJpg, watermarkPdf, signPdf, redactPdf, addPageNumbers, pdfToPdfa, comparePdfs,
  performOcr,
  // Extended converters
  pdfToTxt, pdfToMarkdown, pdfToJson, pdfToXml, pdfToCsv,
  pdfToEpub, pdfToPptx, pdfToExcel,
  heicToJpg, gifToPdf, markdownToPdf, csvToPdf, htmlToPdf, svgToPdf,
};
